...
//yeganlin_20140102,set system device to ro if it was write protected
	if(mmc_card_mmc(card)){
		...
		//set_partition_ro(card,md,"sbl1",5); //sbl1+DDR+rpm+tz+dbi,
		set_partition_ro(card,md,"GPT",5); //sbl1+DDR+rpm+tz+dbi,
		set_partition_ro(card,md,"aboot",6); //aboot+modem+boot+recovery+splash+tombstones
