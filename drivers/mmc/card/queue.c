...
retry:
		blk_queue_max_segments(mq->queue, host->max_segs);
             ...
             if(mmc_card_sd(card) && g_sg0 != NULL){
                 pr_err("ygl mmc_init_queue()::%s:g_sg0 had alloced\n",mmc_hostname(host));
                 mqrq_cur->sg = g_sg0;
                 sg_init_table(g_sg0, host->max_segs);
             }else{
                 pr_err("ygl mmc_init_queue()::%s:g_sg0 had not alloced and alloc here\n",mmc_hostname(host));
            ...
            if(mmc_card_sd(card) && g_sg1 != NULL){
                 pr_err("ygl mmc_init_queue()::%s:g_sg1 had alloced\n",mmc_hostname(host));
                 mqrq_prev->sg = g_sg1;
                 sg_init_table(g_sg1, host->max_segs);
            }else{
                 pr_err("ygl mmc_init_queue()::%s:g_sg1 had not alloced and alloc here\n",mmc_hostname(host));
