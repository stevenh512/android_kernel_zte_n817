...
//yeganlin,20130131,added for alloc memory for struct of scatterlist failure
#ifdef CONFIG_MMC_MSM_CARD_PRE_ALLOC_MEMORY_FOR_SG
      if( host->mmc->index == 1) {// yeganlin,for SD CARD
          pr_err("ygl sdhci_msm_probe()::alloc for sg\n");
          g_sg0 = kmalloc(sizeof(struct scatterlist)*host->mmc->max_segs, GFP_KERNEL);;
          if (ret)
              pr_err("ygl msmsdcc_probe()::alloc for g_sg0 failed!go on and alloc at Queue.c again.\n");
          g_sg1 = kmalloc(sizeof(struct scatterlist)*host->mmc->max_segs, GFP_KERNEL);;
          if (ret)
