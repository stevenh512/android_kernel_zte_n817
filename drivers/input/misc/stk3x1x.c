...
#define DRIVER_VERSION  "3.8.0 for MP Cali"
...
//====
struct stk_cfg * stk_cfgp;
static dev_t stk_dev_number;
struct class *stk_class;
#define STK_MAX_NUM_DEVICES		3
#define STK_DEVICE_NAME		"stk"
//====
...
#endif
#ifdef MP_CALIBRATION
//static void calibration_lightsnesor(struct stk3x1x_data *ps_data, int lightbox_value); //在已知光源下, 校正 ALS 透光率(transmittance)
static int calibration_psnesor(struct stk3x1x_data *ps_data);	//在沒有物體遮擋整機下, 校正 PS 漏光值(crosstalk)
extern void store_transmittance_to_nvitem(uint32_t data);	//儲存 ALS 校正值(transmittance)
extern void store_crosstalk_to_nvitem(uint16_t data);	//儲存 PS 校正值(crosstalk)
extern uint32_t get_transmittance_from_nvitem(void);	//回讀 ALS 校正值(transmittance)
extern uint16_t get_crosstalk_from_nvitem(void);	//回讀 PS 校正值(crosstalk)
#endif
...
static long stk_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	ps_data = container_of(file->f_dentry->d_inode->i_cdev, struct stk3x1x_data, cdev);

	switch(cmd){
		case STK_IOCTL_ALS_ON: //used
			printk("STK,ioctl STK_IOCTL_ALS_ON\n");
			mutex_lock(&ps_data->io_lock);
			stk3x1x_enable_als(ps_data, 1);
			mutex_unlock(&ps_data->io_lock);
			break;
			
		case STK_IOCTL_ALS_OFF: //used
			printk("STK,ioctl STK_IOCTL_ALS_OFF\n");
			mutex_lock(&ps_data->io_lock);
			stk3x1x_enable_als(ps_data, 0);
			mutex_unlock(&ps_data->io_lock);
			break;
			
		case STK_IOCTL_ALS_DATA:
			printk("STK,ioctl STK_IOCTL_ALS_DATA\n");
			break;
			
		case STK_IOCTL_ALS_CALIBRATE:
			printk("STK,ioctl STK_IOCTL_ALS_CALIBRATE\n");
			break;
			
		case STK_IOCTL_CONFIG_GET: //used
			printk("STK,ioctl STK_IOCTL_CONFIG_GET\n");
			ret = copy_to_user((struct stk_cfg *)arg, stk_cfgp, sizeof(struct stk_cfg));
			if (ret) {
		...
		case STK_IOCTL_CONFIG_SET: //used
			printk("STK,ioctl STK_IOCTL_CONFIG_SET: prox_pulse_cnt =%d\n",stk_cfgp->prox_pulse_cnt);
			ret = copy_from_user(stk_cfgp, (struct stk_cfg *)arg, sizeof(struct stk_cfg));
			if (ret) {
				printk(KERN_ERR "TAOS: copy_from_user failed in ioctl config_set\n");
				return -ENODATA;
			}
			if(0x20 == stk_cfgp->prox_gain)
				stk_cfgp->prox_pulse_cnt = 0;
			
			stk_cfgp->prox_gain = 3; //STK, fix value
			stk_cfgp->prox_threshold_lo = stk_cfgp->prox_threshold_hi - ps_data->stk_ht_n_ct + ps_data->stk_lt_n_ct;
			reg_value = reg_value | (stk_cfgp->prox_gain << 4) | (stk_cfgp->prox_pulse_cnt);
			ps_data->psctrl_reg = reg_value;
			ret = stk3x1x_i2c_write_data(ps_data->client, STK_PSCTRL_REG, 1,&reg_value);
			if(ret < 0)
			{
		...
		case STK_IOCTL_PROX_ON: //used
			printk("STK,ioctl STK_IOCTL_PROX_ON\n");
			mutex_lock(&ps_data->io_lock);
			stk3x1x_enable_ps(ps_data, 1, 1);
			mutex_unlock(&ps_data->io_lock);
			break;
			
		case STK_IOCTL_PROX_OFF: //used
			printk("STK,ioctl STK_IOCTL_PROX_OFF\n");
			mutex_lock(&ps_data->io_lock);
			stk3x1x_enable_ps(ps_data, 0, 1);
			mutex_unlock(&ps_data->io_lock);
			break;
			
		case STK_IOCTL_PROX_DATA:
			printk("STK,ioctl STK_IOCTL_PROX_DATA\n");
			break;

		case STK_IOCTL_PROX_EVENT:
			printk("STK,ioctl STK_IOCTL_PROX_EVENT\n");
			break;
			
		case STK_IOCTL_PROX_CALIBRATE: //used
			printk("STK,ioctl STK_IOCTL_PROX_CALIBRATE\n");
			ret = calibration_psnesor(ps_data); //ret < 0 fail; ret >=0 success
			if(ret < 0){
				printk("PS calibration fali %s\n",__func__);
				return ret;
			}
			printk("============> ret = %ld \n",ret);
			stk_cfgp->prox_threshold_hi = PS_THDH;
			stk_cfgp->prox_pulse_cnt = PS_CMD & 0X0F;
			if(0 == stk_cfgp->prox_pulse_cnt){
				stk_cfgp->prox_gain = 0x20;
				stk_cfgp->prox_pulse_cnt = 1;
			} else {
				stk_cfgp->prox_gain = 0x60; //default value 0x60 match TAOS, not used
			}
			break;
			
		case STK_IOCTL_PROX_GET_ENABLED: //used
			printk("STK,ioctl STK_IOCTL_PROX_GET_ENABLED\n");
			if(ps_data->ps_enabled == true)
				prox_enable = 1;
			else
				prox_enable = 0;
			return put_user(prox_enable, (unsigned long __user *)arg);
			break;
			
		case STK_IOCTL_ALS_GET_ENABLED: //used
			if(ps_data->als_enabled == true)
				als_enable = 1;
			else
				als_enable = 0;
			return put_user(als_enable, (unsigned long __user *)arg);
			printk("STK,ioctl STK_IOCTL_ALS_GET_ENABLED\n");
			break;
			
		default:
			printk("STK,ioctl default!\n");
			return -EINVAL;
			break;
	}
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
    ...
    if(enable)
	{
		...
		if(!validate_reg)		
		{
			...
			input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, 5);
...
#endif /* #ifdef STK_CHK_REG */
		{
			...
			input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, near_far_state*5);
			input_sync(ps_data->ps_input_dev);
			wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);
			reading = stk3x1x_get_ps_reading(ps_data);
...
#if 0
static void calibration_lightsnesor(struct stk3x1x_data *ps_data, int lightbox_value)  //lightbox_value = 已知的校準光源亮度
{
	int32_t reading, mean, flag_reg, count = 0;
	uint32_t new_als_transmittance = 0;	
	int sum = 0;
	...
	while(count<5)
	{
		...
		if(reading < 0)		
		{
			printk(KERN_INFO "%s: read als error %d\n",__func__, reading);
			return;
		}
		
		printk(KERN_INFO "%s: read als data[%d] = %d\n",__func__, count, reading);
		count = count + 1;
		sum = sum + reading;
	}
	...
	new_als_transmittance = (mean * 1100) / lightbox_value;
	
	ps_data->als_transmittance = new_als_transmittance;
	
	store_transmittance_to_nvitem(new_als_transmittance);	//保存 transmittance 到非揮發記憶體空間
...
static int calibration_get_crosstalk_ps(struct stk3x1x_data *ps_data)
{
	...
	crosstalk = sum/5;
	printk("%s: get crosstalk = %d\n",__func__,crosstalk);
	return crosstalk;
	
}

static int calibration_psnesor(struct stk3x1x_data *ps_data)  //校正整機漏光(校正時請確保整機沒有物體遮擋)
{
	...
	ps_cmd = stk3x1x_i2c_smbus_read_byte_data(ps_data->client,STK_PSCTRL_REG);
	if(ps_cmd < 0)
	{
		printk(KERN_ERR "%s: fail, ps_cmd=%d\n", __func__, ps_cmd);
		return -1;
	}
	
	while(((ps_val < 60) ||(ps_val > 500))&& (ps_cmd >= 0x30) && (ps_cmd <= 0x34))
	{
		ps_cmd = stk3x1x_i2c_smbus_read_byte_data(ps_data->client,STK_PSCTRL_REG);
		if(ps_cmd < 0)
		{
		...
		val[0] = ps_cmd;
		ret = stk3x1x_i2c_write_data(ps_data->client, STK_PSCTRL_REG, 1,val );
		if(ret < 0)
		{
			printk(KERN_ERR "%s: write i2c error\n", __func__);
			return ret;
		...
		printk("========>STK calibrate,ps_val = %d, ps_cmd = %d",ps_val,(ps_cmd & 0x0F));
	...
	PS_THDH = ps_data->ps_thd_h;
	stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
	stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
	PS_CMD = (ps_cmd & 0xFF) ;
	
	ret = stk3x1x_i2c_write_data(ps_data->client, STK_PSCTRL_REG, 1,(unsigned char *)&PS_CMD );
	if(ret < 0)
	{
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	...
	printk("STK P sensor calibrate success! \n");
	return 0;
}
...
static ssize_t stk_ps_distance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, dist*5);
	input_sync(ps_data->ps_input_dev);
	wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);
...
static ssize_t stk_ps_distance_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, value*5);
	input_sync(ps_data->ps_input_dev);
	wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);	
...
static ssize_t stk_ps_cali_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	it_ps = PS_CMD & 0X0F;
	gain_ps = ((ps_data->psctrl_reg) & 0x30) >> 4 ;

	printk("P sensor calibrate data: hi=%d, it_ps=%d, gain=%d\n",PS_THDH,it_ps, gain_ps);
	
	return scnprintf(buf, PAGE_SIZE, "%d,%d,%d\n",PS_THDH,it_ps, 0x60); //0x60 the default value match TAOS
}
static ssize_t stk_ps_cali_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	int ret = 0;
	//ret = kstrtoul(buf, 10, &value);
	printk("%s: into P sensor calibration \n",__func__);
...
static ssize_t stk_ps_cali_get_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);	
	
	return scnprintf(buf, PAGE_SIZE, "%d\n", ps_data->stk_max_min_diff);     		
}
static ssize_t stk_ps_cali_get_data_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);	
    //unsigned long value = 0;
	int ret;
	int i,j;
	unsigned char val[100] = {0};
	...
	scnprintf(val, PAGE_SIZE, "%s\n", buf);
	printk("%s:into set calibration data, insert string is %s\n",__func__,tmp);
	...
	for(i = 0, j = 0; *tmp != '\0' && i <= 3 && j < 20; ) {
		if(*tmp  == ',') {
			*(cal_data_char[i] +j) = '\0';
			...
			j = 0;
			if (i >3 ) {
				printk("sensor_hal: array bounds!\n");
				return 0;
			}
		}
		if((*tmp >= '0') && (*tmp <= '9')) {
	...
	ps_data->ps_thd_h = simple_strtoul(cal_data_char[0], NULL, 10);
	prox_it_time = simple_strtoul(cal_data_char[1], NULL, 10);
	//prox_gain = simple_strtoul(cal_data_char[2], NULL, 10);
	prox_gain = 3; //fix gain value : 3

#if 0
	ret = stk3x1x_i2c_read_data(ps_data->client, STK_PSCTRL_REG, 1, &reg_value);
	if(ret < 0)
	{
		printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
		return ret;
	...
	reg_value = reg_value | (prox_gain << 4) |(prox_it_time);
	ps_data->psctrl_reg = reg_value;
	ret = stk3x1x_i2c_write_data(ps_data->client, STK_PSCTRL_REG, 1,&reg_value);
	if(ret < 0)
	{
	...
	PS_THDH = ps_data->ps_thd_h;
	ps_data->ps_thd_l = PS_THDH - ps_data->stk_ht_n_ct + ps_data->stk_lt_n_ct;
	
	//PS_THDH,ps_data->ps_thd_l, check range
	stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
	stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);

	printk("%s:into set calibration data, PS_THDH=%d, low=%d, it-time=%d, reg=%d\n",__func__,
				PS_THDH,ps_data->ps_thd_l,prox_it_time,reg_value);
...
//static struct device_attribute ps_cali_attribute = __ATTR(calibrate,0666,stk_ps_cali_show,stk_ps_cali_store);
//static struct device_attribute ps_cali_get_data_attribute = __ATTR(set_calibrate_data,0666,stk_ps_cali_get_data_show,stk_ps_cali_get_data_store);
#ifdef STK_TUNE0
...
static DEVICE_ATTR(calibrate, 0666,
		stk_ps_cali_show,
		stk_ps_cali_store);
static DEVICE_ATTR(set_calibrate_data, 0666,
		stk_ps_cali_get_data_show,
		stk_ps_cali_get_data_store);
#endif

#ifdef STK_TUNE0
#if 0
static struct attribute *psensor_cali[] =
{
	&ps_cali_attribute.attr,
	&ps_cali_get_data_attribute.attr,
};

static struct attribute_group stk_psensor_cali_group = {
	.attrs = psensor_cali,
};
#endif
static struct attribute *psensor_cali[] = {
...
static const struct attribute_group stk_psensor_cali_group = {
	.attrs = psensor_cali,
};
#endif
...
#ifdef MP_CALIBRATION
		if(ps_data->cali_ct != 0xFFFF)
		{
			ps_data->ps_high_thd_boot = ps_data->cali_ct + ps_data->stk_ht_n_ct*3;
			ps_data->ps_low_thd_boot = ps_data->cali_ct + ps_data->stk_lt_n_ct*3;
			ps_data->ps_thd_h = ps_data->ps_high_thd_boot ;
			ps_data->ps_thd_l = ps_data->ps_low_thd_boot ;	
			stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
			stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
			printk(KERN_INFO "%s: set boot cali HT=%d, LT=%d, ct=%d\n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l, ps_data->cali_ct);
		}
#endif			
		hrtimer_cancel(&ps_data->ps_tune0_timer);	
		return 0;
...
#ifdef MP_CALIBRATION
	if((ps_data->ps_stat_data[1] > (ps_data->cali_ct + STK_CT_DELTA_MAX)) && (ps_data->cali_ct != 0xFFFF))
	{
		printk(KERN_INFO "%s: reset boot ct %d to %d(cali)\n", __func__, ps_data->ps_stat_data[1], ps_data->cali_ct);
		ps_data->ps_stat_data[1] = ps_data->cali_ct;
	}
#endif
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	{
#ifdef MP_CALIBRATION
		if((ps_data->cali_ct != 0xFFFF) && (ps_data->ps_thd_h != (ps_data->cali_ct + ps_data->stk_ht_n_ct)))
		{
			ps_data->ps_thd_h = ps_data->cali_ct + ps_data->stk_ht_n_ct;
			ps_data->ps_thd_l = ps_data->cali_ct + ps_data->stk_lt_n_ct;
			stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
			stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
			printk(KERN_INFO "%s: set cali HT=%d, LT=%d, ct=%d,%d\n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l, ps_data->psi, ps_data->cali_ct);
		}
#endif	
		return 0;
	}
...
#ifdef MP_CALIBRATION
		if((ps_data->psi > (ps_data->cali_ct + STK_CT_DELTA_MAX)) && (ps_data->cali_ct != 0xFFFF))
		{
			ps_data->psi_set = ps_data->cali_ct;
			ps_data->ps_thd_h = ps_data->cali_ct + ps_data->stk_ht_n_ct;
			ps_data->ps_thd_l = ps_data->cali_ct + ps_data->stk_lt_n_ct;		
			printk(KERN_INFO "%s: update cali HT=%d, LT=%d, ct=%d,%d\n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l, ps_data->psi, ps_data->cali_ct);
		}
#endif
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	//if(abs(ps_data->als_lux_last - reading_lux) >= STK_ALS_CHANGE_THD)  //zxf
	{
...
{		
	...
	//add irq wake up function
#if 0
	err = enable_irq_wake(irq);
	if (err) {
		printk(KERN_CRIT "stk enable_irq_wake failed\n");
        return err;
    }
#endif
                        ...
                        const struct i2c_device_id *id)
{
	...
	//
	stk_cfgp = kmalloc(sizeof(struct stk_cfg), GFP_KERNEL);
	if(!stk_cfgp){
		printk(KERN_ERR "%s: kmalloc for struct taos_cfg failed\n",__func__);
		return -ENOMEM;
	}
	memset(stk_cfgp, 0, sizeof(struct stk_cfg));
	//
...
//add cdev
	err = alloc_chrdev_region(&stk_dev_number, 0, STK_MAX_NUM_DEVICES, STK_DEVICE_NAME);
	if(err < 0) {
		printk(KERN_ERR "%s: alloc_chrdev_region() failed \n",__func__);
		//return err;
		goto err_alloc_chrdev_region;
	}
	stk_class = class_create(THIS_MODULE, STK_DEVICE_NAME);
	cdev_init(&ps_data->cdev, &stk_fops);
	ps_data->cdev.owner = THIS_MODULE;
	err = cdev_add(&ps_data->cdev, stk_dev_number, 1);
	if(err < 0) {
...
//add cdev end
...
#ifdef MP_CALIBRATION
	//ps_crosstalk = get_crosstalk_from_nvitem();	//開機 probe 過程中回讀 p-sensor 校準值, 做初始化設定
	if( ps_crosstalk > 0 && ps_crosstalk != 0xFFFF )
	{
		ps_data->cali_ct = ps_crosstalk;
		printk(KERN_INFO "%s:PS Calibration successfully, crosstalk value = %d\n",__func__, ps_data->cali_ct);
	}
	else
	{
		ps_data->cali_ct = 0xFFFF;
		printk(KERN_INFO "%s:PS No calibration, default crosstalk value = %d\n",__func__, ps_data->cali_ct);
	}
	
	//new_als_transmittance = get_transmittance_from_nvitem();	//開機 probe 過程中回讀 light-sensor 校準值, 做初始化設定
	if( new_als_transmittance > 0 )
	{
		ps_data->als_transmittance = new_als_transmittance;
		printk(KERN_INFO "%s:ALS Calibration successfully, transmittance value = %d\n",__func__, ps_data->als_transmittance);
	}
	else
	{
		ps_data->als_transmittance = plat_data->transmittance;
		printk(KERN_INFO "%s:ALS No calibration, default transmittance value = %d\n",__func__, ps_data->als_transmittance);
	}
#else
	ps_data->als_transmittance = plat_data->transmittance;
#endif
	...
	device_create(stk_class, NULL, MKDEV(MAJOR(stk_dev_number), 0), &stk_ps_driver ,"stk");

#ifdef STK_TUNE0
    err = sysfs_create_group(&client->dev.kobj, &stk_psensor_cali_group);
	if (err < 0) 
	{
		printk(KERN_ERR "%s:could not create sysfs group for ps\n", __func__);
		goto err_creat_sysfs_psensor_cali;
	}
#endif	
...
#ifdef STK_TUNE0
err_creat_sysfs_psensor_cali:
	sysfs_remove_group(&client->dev.kobj, &stk_psensor_cali_group);
#endif

class_destroy(stk_class);
...
#endif
err_cdev_add:
	cdev_del(&ps_data->cdev);
	device_destroy(stk_class, MKDEV(MAJOR(stk_dev_number), 0));
err_alloc_chrdev_region:
	unregister_chrdev_region(stk_dev_number, STK_MAX_NUM_DEVICES);
