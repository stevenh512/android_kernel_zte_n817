...
//#include "msg22xx_0102_update_bin.h" // for MSG22xx
//#include "msg22xx_yyyy_update_bin.h"
#include "msg22xx_0108_update_bin.h" // for MSG22xx
...
extern 	struct wake_lock Proximity_wake_lock;			//zte add
...
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
                printk("g_EnableTpProximity = %d, pPacket[5] = 0x%x\n", g_EnableTpProximity, pPacket[5]);

                if (g_EnableTpProximity && ((pPacket[5] == 0x80) || (pPacket[5] == 0x40)))
                {
                	wake_lock_timeout(&Proximity_wake_lock,HZ/4*3);
                    if (pPacket[5] == 0x80) // close to
                    {
                        g_FaceClosingTp = 1;
						// Proximity-start-
                        input_report_abs(g_ProximityInputDevice, ABS_DISTANCE, 0); // report near
                    ...
                    else if (pPacket[5] == 0x40) // far away
                    {
                        g_FaceClosingTp = 0;
						// Proximity-start-
                        input_report_abs(g_ProximityInputDevice, ABS_DISTANCE, 1); // report near
                        input_sync(g_ProximityInputDevice);
						// Proximity-end-
                    }
...
static void _DrvFwCtrlStoreFirmwareData(const u8 *pBuf, u32 nSize)
{
...
Msg22xxSwId_e eSwId_N817=2;
u16 nMajor_N817 = 0, nMinor_N817 = 0;
u8 *pVersion_N817 = NULL;
...
void _DrvFwCtrlMsg21xxaCheckFirmwareUpdateBySwId(void) 
{
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        if (eSwId == MSG21XXA_SW_ID_XXXX)
        {
            ...
            nUpdateBinMajor = msg22xx_xxxx_update_bin[0x7F4F]<<8 | msg22xx_xxxx_update_bin[0x7F4E];
            nUpdateBinMinor = msg22xx_xxxx_update_bin[0x7F51]<<8 | msg22xx_xxxx_update_bin[0x7F50];
#endif
        }
        /*
...
static s32 _DrvFwCtrlUpdateFirmwareBySdCard(char *pFilePath)
{
    s32 nRetVal = 0;
...
s32 DrvFwCtrlUpdateFirmwareBySdCard(char *pFilePath)
{
    s32 nRetVal = -1;
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        else //touch on screen
        {
            if (tInfo.nTouchKeyCode != 0)
            {
				...
				DrvPlatformLyrFingerTouchPressed(key_x, key_y, 0, 0);
