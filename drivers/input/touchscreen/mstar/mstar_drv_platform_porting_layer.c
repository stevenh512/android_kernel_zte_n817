...
struct sensors_classdev proximity_cdev;
static struct sensors_classdev sensors_proximity_cdev = {
    .name = "msg22xx-proximity",
    .vendor = "MStar",
    .version = 1,
...
static ssize_t msg22xx_proximity_enable_show(struct device *dev,
                                             struct device_attribute *attr,
                                             char *buf)
{
    extern u8 g_EnableTpProximity;
    DBG("***msg22xx proximity state = %s***\n", g_EnableTpProximity ? "open" : "close");
    return sprintf(buf, "%s\n", g_EnableTpProximity ? "open" : "close");
}

static ssize_t msg22xx_proximity_enable_store(struct device *dev,
                                              struct device_attribute *attr,
                                              const char *buf,
                                              size_t size)
{
    if (buf != NULL) {
        if (sysfs_streq(buf, "1"))
            DrvPlatformLyrTpPsEnable(&proximity_cdev, 1);
	else if (sysfs_streq(buf, "0"))
            DrvPlatformLyrTpPsEnable(&proximity_cdev, 0);
    }

    return size;
}

static struct device_attribute ps_enable_attribute = __ATTR(enable, 0664, msg22xx_proximity_enable_show, msg22xx_proximity_enable_store);

static struct attribute *msg22xx_ps_attrs [] =
{
    &ps_enable_attribute.attr,
    NULL
};

static struct attribute_group msg22xx_ps_attribute_group = {
	.name = "driver",
	.attrs = msg22xx_ps_attrs,
};
// Proximity-end-
...
const int g_TpVirtualKey[] = {TOUCH_KEY_BACK, TOUCH_KEY_HOME, TOUCH_KEY_MENU};
...
/*
 * TODO: Add enable vreg(s) from qcom MSG2133A
 */
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(void)
{
    ...
    nRetVal = regulator_set_voltage(g_ReguVdd, 2850000, 2850000); // For specific SPRD BB chip(ex. SC7715) or QCOM BB chip(ex. MSM8610), need to enable this function call for correctly power on Touch IC.

    if (nRetVal)
    {
        DBG("Could not set to 2850mv.\n");
    }
...
struct wake_lock Proximity_wake_lock; 			//zte add
...
// Proximity-start-
    ...
    if (!g_ProximityInputDevice) {
        DBG("failed to allocate input proximity\n");
        return -ENOMEM;
    }

    g_ProximityInputDevice->name = "msg22xx-ps";
    ...
    if (nRetVal) {
        DBG("failed to register input proximity %d\n", nRetVal);
        return -ENOMEM;
    }
    ...
    nRetVal = sysfs_create_group(&g_ProximityInputDevice->dev.kobj, &msg22xx_ps_attribute_group);
    if (nRetVal < 0)
    {
        DBG("%s:could not create sysfs group for ps\n", __func__);
        return -ENOMEM;
    }

    input_set_drvdata(g_ProximityInputDevice, NULL);
	wake_lock_init(&Proximity_wake_lock, WAKE_LOCK_SUSPEND, "Proximity_wake_lock");	 //zte add
    ...
    if (nRetVal) {
        DBG("failed to register sensor device class %d\n", nRetVal);
        return -ENOMEM;
    }
#endif // CONFIG_ENABLE_PROXIMITY_DETECTION
// Proximity-end-
...
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
// Proximity-start-
int DrvPlatformLyrTpPsEnable(struct sensors_classdev* proximity_cdev, unsigned int nEnable)
// Proximity-end-
{
    printk("*** %s() nEnable = %d ***\n", __func__, nEnable);
