...
static int ft5x0x_GetFirmwareSize(char * firmware_name)
{
	...
	sprintf(filepath, "/etc/firmware/%s", firmware_name);
...
int focaltech_fwupdate(struct i2c_client *client, char *fwfilename )
{
	...
	if(0<=ft5x0x_i2c_Read(client, &reg, 1,&crtFwVendorId,1)){
...
int focaltech_get_fw_ver(struct i2c_client *client, char *fwfilename )
{
	...
	fwsize = ft5x0x_GetFirmwareSize(fwfilename);
	if(fwsize <= 0)
	{
	...
	if(ft5x0x_ReadFirmware(fwfilename, pbt_buf))
	{
