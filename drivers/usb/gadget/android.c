...
#endif
#include "f_ncm.c"
/* Charger begin --zhenghuan 2015-3-23*/
#include "f_charger.c"
/* Charger end --zhenghuan 2015-3-23*/
...
{
	if (is_ftm_mode() || is_pid_configed_from_file() ||zte_usb_parameters.noSerialno ) {
...
static bool is_valid_function(char *functions)
{
	...
	while (functions) {
		...
		/*serial, only single smd channel is allowed*/
			...
			printk(KERN_ERR"xbl: serial_transports = %s ,include invalid function\n",serial_transports);
			return false;
		}
...
DESCRIPTOR_STRING_ATTR(iMeid, meid_string)
