...
static int pkt_pass_permission_check(unsigned char * buf)
{
	...
	31: Request for msg report(+)
        53: General voice call(+)
	66: Invalid nv_read/write because SP is locked(+)
	93: Extended logmask for > 32 bits(+)
	121:Request for extended msg report(+)
	39: Nonvolatile Item Write(-)
	0x3A: Restart Command
	********************************/
	if (((*buf >= 2) && (*buf <= 11)) ||
		((*buf >= 15) && (*buf <= 18)) ||
		(*buf == 31) ||/* (*buf == 66) || */
        ...
        75 15 27 00: WCDMA voice call(+)
        75 15 28 00: GSM voice call(+)
        **********************/
	if (*buf == 75 ) {
		if ((*(buf+1) == 15)) {
			if ((*(uint16_t *)(buf+2) == 05) ||
			    (*(uint16_t *)(buf+2) == 06) || 
			    (*(uint16_t *)(buf+2) == 27) || 
			    (*(uint16_t *)(buf+2) == 28))
				return 0;
