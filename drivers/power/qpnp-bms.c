				...
				int batt_temp, int target_soc)
{
	...
	if (target_cc_uah < CC_STEP_INCREMENT_UAH) {
		/*
					...
					int batt_temp)
{
	...
	soc = DIV_ROUND_CLOSEST((remaining_usable_charge_uah * 100),
				(params->fcc_uah - params->uuc_uah-30000));
#else
	soc = DIV_ROUND_CLOSEST((remaining_usable_charge_uah * 100),
				(params->fcc_uah - params->uuc_uah));
#endif
...
static int get_prop_bms_capacity(struct qpnp_bms_chip *chip)
{
	...
	if(soc == 100 && is_battery_charging(chip)) {
		pr_info("zte adjust from 100->99\n");
		soc = 99;
	}
	pr_debug("bms report soc=%d\n",soc);
	return soc;
}
#endif
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	if (battery_id < 0) {
		pr_err("zte charge read battery id err = %lld\n", battery_id);
		return battery_id;
	}
	printk("zte charge read battery_id NO USED =0x%llX,FCC=%d\n",battery_id,palladium_data->fcc);
	...
	printk("set_battery_data  max_voltage_uv=%d,cutoff_uv=%d,iterm_ua =%d, fcc_sf_lut=0x%x\n", \
		chip->max_voltage_uv,chip->v_cutoff_uv,chip->chg_term_ua, (int)chip->fcc_sf_lut);
