...
qpnp_chg_dc_dcin_valid_irq_handler(int irq, void *_chip)
{
	...
	printk("%s, dc is  %s\n", __func__, dc_present?"present":"not present");
...
static bool extovp_on_while_chg = false;
extern bool ac_charger_control_lpmclock;
static irqreturn_t
qpnp_chg_chgr_chg_fastchg_irq_handler(int irq, void *_chip)
{
	...
	//zte add
	if (ext_ovp_present) {
		int fast_chg_on = qpnp_chg_is_fastchg_on(chip);
			...
			if (fast_chg_on)
			{
				if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
					...
					if ((ret.intval / 1000) > USB_WALL_THRESHOLD_MA	|| socinfo_get_ftm_flag())
					{
						...
						zte_ext_ovp_enable();
						extovp_on_while_chg = true;
						ac_charger_control_lpmclock = true;
						pr_info("setting ac_charger_control_lpmclock true\n");
					}
				}
			} else {
				...
				if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
					int mA = qpnp_chg_usb_iusbmax_get(chip);
					if ((mA <= 500) && (extovp_on_while_chg))
					{
							qpnp_chg_iusbmax_set(chip, mA*3);
							pr_info("ext-ovp disable due to full charge, set iusbnax to %dmA\n",mA*3);
					}
				}
				extovp_on_while_chg = false;
				ac_charger_control_lpmclock = false;
				pr_info("setting ac_charger_control_lpmclock false\n");
			}
...
{
	int rc;
	printk("%s,chip->charging_disabled=%d,enable=%d\n",__func__,chip->charging_disabled,enable);
...
get_prop_capacity(struct qpnp_chg_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		printk("%s,battery_v=%d,soc=%d \n",__func__,battery_v,soc);
		...
		if (((soc <3)&&(battery_v <3400000))||((soc <5)&&(battery_v <3300000))||(battery_v<3200000))
		{
		...
		if (soc == 0) {
			...
			//zte add
			if((!enable_to_shutdown)||(battery_v>3550000))
			{
				g_bms_flag=100;
				return 1;
			}
...
bool	low_chg_current	=	true;	//zte jiangfeng add
static void
qpnp_chg_set_appropriate_battery_current(struct qpnp_chg_chip *chip)
{
	...
	//zte jiangfeng add
		if(low_chg_current)
		{
			chg_current = min(chg_current, (unsigned int)1000);
		}
	//zte jiangfeng add, end
...
//zte jiangfeng add
void zte_set_battery_current(bool vlow_chg_current)
{
	low_chg_current	=	vlow_chg_current;
	if(zte_chip)
		qpnp_chg_set_appropriate_battery_current(zte_chip);
}
EXPORT_SYMBOL(zte_set_battery_current);
//zte jiangfeng add, end
...
#define ZTE_VBAT_TOLERANCE_MV  70	//zte jiangfeng add
...
qpnp_eoc_work(struct work_struct *work)
{
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		printk("ibat_ma: %d term_current =%d,vbat_mv=%d\n",
			ibat_ma, chip->term_current,vbat_mv);

		//zte jiangfeng add
		if(vbat_mv<(chip->max_voltage_mv-ZTE_VBAT_TOLERANCE_MV))
		{
			count = 0;
		}
		//zte jiangfeng add, end
...
qpnp_chg_insertion_ocv_work(struct work_struct *work)
{
	...
	printk("%s,chip->charging_disabled=%d\n",__func__,chip->charging_disabled);
	rc = qpnp_chg_read(chip, &bat_if_sts, INT_RT_STS(chip->bat_if_base), 1);
	if (rc)
		pr_err("failed to read bat_if sts %d\n", rc);
...
#define	ZTE_HOT_TEMP_DEFAULT	520
#elif defined(CONFIG_BOARD_DEMI) || defined(CONFIG_BOARD_BONNIE) || defined(CONFIG_BOARD_EROS) \
	|| defined(CONFIG_BOARD_WELLINGTON) || defined(CONFIG_BOARD_FAERIE)  || defined(CONFIG_BOARD_IRIS)
#define	ZTE_HOT_TEMP_DEFAULT	460
#else
...
qpnp_chg_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	printk("temp = %d bat_cool = %d,bat_warm = %d, bat_hot = %d\n",temp,bat_cool,bat_warm,bat_hot);
#if 0
		...
		case SMBCL_CHGR_SUBTYPE:
			...
			if (ext_ovp_present) {
				rc |= devm_request_irq(chip->dev, chip->chg_fastchg.irq,
						...
						IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,//zte add IRQF_TRIGGER_FALLING
						"fast-chg-on", chip);
			} else {
				rc |= devm_request_irq(chip->dev, chip->chg_fastchg.irq,
						qpnp_chg_chgr_chg_fastchg_irq_handler,
...
extern uint8_t get_zte_hw_ver_byte(void);
static int
...
#if defined(CONFIG_BOARD_DEMI)
	if(0 == get_zte_hw_ver_byte())
	{
		chip->hot_batt_p = 25;
		chip->cold_batt_p = 80;
