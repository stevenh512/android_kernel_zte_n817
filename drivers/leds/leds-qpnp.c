...
void update_led_state(struct qpnp_led_data *led)
{
	...
	if(DEBUG_LED)
		printk(LED_DRIVER_TAG "LedState:red=%d green=%d \n",red_led_opened,green_led_opened);
}
...
static int qpnp_mpp_set(struct qpnp_led_data *led)
{
	...
	update_led_state(led);		//zte jiangfeng add //only 1 pwem in 8610 LED for red/green
#endif

	if (led->cdev.brightness) {
		/*  error log: "qcom,leds-qpnp-ee30e000: brightness is less ... " zte-ccb-20130925
		...
		{		
			...
			//rc = pwm_config_us(led->mpp_cfg->pwm_cfg->pwm_dev,250000,1000000);

			#ifdef CONFIG_BOARD_DEMI
			rc = pwm_config_us(led->mpp_cfg->pwm_cfg->pwm_dev,500000,3*1000000);//P821A16-KK 20140616 zte
			#else
			rc = pwm_config_us(led->mpp_cfg->pwm_cfg->pwm_dev,250000,7.5*1000000);//blink 0.25s per 7.5s by liyuanfei
			#endif
...
static void qpnp_led_turn_off_delayed(struct work_struct *work)
{
	...
	printk("qpnp_led_turn_off_delayed\n");
			...
			struct pwm_config_data *pwm_cfg)
{
	...
	if (pwm_cfg->use_blink) {
		...
		if(DEBUG_LED)
			printk(LED_DRIVER_TAG "led_blink:%s  br=%d  blink=%d\n",led->cdev.name,led->cdev.brightness,led->cdev.blink_value);	//ZTE LOG
		pwm_free(pwm_cfg->pwm_dev);
		qpnp_pwm_init(pwm_cfg, led->spmi_dev, led->cdev.name);
		...
		//qpnp_led_set(&led->cdev, led->cdev.brightness);		//zte jiangfeng remove
	}
	#else
	if (pwm_cfg->use_blink) {
		if (led->cdev.brightness) {
...
static ssize_t current_mpp_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	struct qpnp_led_data *led;
	...
	ret = kstrtouint(buf, 10, &current_i);
	if (ret)
		return ret;
	
	printk(LED_DRIVER_TAG "current_mpp_store(%d) \t %s\n",current_i,led_cdev->name);	//zte-ccb-20130927
	
	led = container_of(led_cdev, struct qpnp_led_data, cdev);

	//val = (led->mpp_cfg->current_setting / LED_MPP_CURRENT_PER_SETTING) - 1;
	val = current_i / LED_MPP_CURRENT_PER_SETTING - 1;
	led->mpp_cfg->current_setting = current_i;
...
static DEVICE_ATTR(current_mpp, 0664, NULL, current_mpp_store);	/* zte-ccb-20130927 add debug fs node */
#else
...
static struct attribute *mpp_attrs[] = {	/* zte-ccb-20130927 add debug fs node */
	&dev_attr_current_mpp.attr,
	NULL
};
#endif
...
static const struct attribute_group mpp_attr_group = {	/* zte-ccb-20130927 add debug fs node */
	.attrs = mpp_attrs,
};
#endif
...
static int __devinit qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		if (led->id == QPNP_ID_LED_MPP) {
		#if !defined CONFIG_BOARD_WELLINGTON
			if(led->mpp_cfg->pwm_mode == MANUAL_MODE){	/* zte-ccb-20130927 add debug fs node */
				rc = sysfs_create_group(&led->cdev.dev->kobj,
					&mpp_attr_group);
				if (rc)
					goto fail_id_check;
