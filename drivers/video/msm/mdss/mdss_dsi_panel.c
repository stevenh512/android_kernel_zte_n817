...
void  mdss_dsi_panel_lcd_proc(struct device_node *np)
{	
	...
	panel_name = of_get_property(np,
		"zte,lcd-proc-panel-name", NULL);
	if (!panel_name){
...
static void mdss_dsi_panel_bklt_gpio(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	level &= 0x1f;
	printk("set backlight to level %d \n",level);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (data) {
		if (!strncmp(data, "bl_ctrl_wled", 12)) {
			...
			if (gpio_request(ctrl_pdata->bl_cmd_gpio, "disp_bkl")) {
