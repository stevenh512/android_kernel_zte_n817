...
extern void zte_set_battery_current(bool vlow_chg_current);
			     ...
			     int op_enable)
{
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on && mfd->mdp.on_fnc) {
			...
			rc=socinfo_get_ffbm_flag();
			printk("%s BOOT_MODE_FFBM ? 1 : 0 rc=%d\n",__func__,rc);
			if(rc==0){
			mdss_fb_set_backlight(mfd, 15);
				}
