				...
				struct mdp_overlay *req)
{
	...
	if (req->id == MSMFB_NEW_REQUEST) {
		if (dma->source_config.stride != stride ||
			...
			//	if(req->src.width==736)
				//	dma->source_config.width = 720;
			dma->source_config.format = format;
					...
					struct mdp_display_commit *cmt_data)
{
	...
	if (mdp3_session->first_commit) {
		...
		msleep(2000 / panel_info->mipi.frame_rate);
		printk("%s first_commit=%d\n",__func__,mdp3_session->first_commit);
		mdp3_session->first_commit = false;
	}
    
	mdp3_session->vsync_before_commit = 0;
	if (reset_done && (panel && panel->set_backlight)){
		#ifdef CONFIG_BOARD_WELLINGTON
		mdelay(30);
		#endif
		panel->set_backlight(panel, panel->panel_info.bl_max/2);
		}
