	...
	if (enable) {
		...
		for (i = num_clk - 1; i >= 0; i--) {
			if (clk_ptr[i] != NULL) {
				...
				clk_put(clk_ptr[i]);
			}else{ /*yichangming 20140305 mclk close*/
				clk_ptr[i] = clk_get(dev, clk_info[i].clk_name);
				CDBG("%s disable %s\n", __func__,
					clk_info[i].clk_name);
				clk_disable(clk_ptr[i]);
...
static int registed_gpio[120] = {0};
	...
	int gpio_en)
{
	...
	if (gpio_en) {
		for (i = 0; i < size; i++) {
			//ZTE_YCM_20140218 avoid request gpio fail alarm
			if(registed_gpio[gpio_tbl[i].gpio])
				break;
			...
			}else //ZTE_YCM_20140218 avoid request gpio fail alarm
				registed_gpio[gpio_tbl[i].gpio] = 1;
		}
	} else {
	#if 0 //wly delete for camera sleep config
		gpio_free_array(gpio_tbl, size);
		for (i = 0; i < size; i++) //ZTE_YCM_20140218 avoid request gpio fail alarm
			registed_gpio[gpio_tbl[i].gpio] = 0;
	#endif 
	}
	return rc;
}
