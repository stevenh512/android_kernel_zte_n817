...
#endif
#if defined (CONFIG_OV8835) //weilanying add actuator calibration on 20140714
void get_otp_af_infor(int * vcm_start, int *vcm_end);
 int af_actuator_sensor_ov8835_flush_cali_afc(struct msm_actuator_ctrl_t *a_ctrl,struct reg_settings_t *init_settings)
{
  int  vcm_start, vcm_end;
  int32_t step_bound_region_0, step_bound_region_1;
  uint16_t step;
  get_otp_af_infor(&vcm_start,  &vcm_end);
 pr_err("%s:vcm_start=%d, vcm_end=%d", __func__, vcm_start, vcm_end);
   ...
   */
  if (vcm_start > 450) {
    step = (uint16_t)((vcm_start - 180) / step_bound_region_0);
  } else if (vcm_start > 380 && vcm_start <= 450) {
    step = (uint16_t)((vcm_start - 160) / step_bound_region_0);
  } else if (vcm_start > 340 && vcm_start <= 380) {
    step = (uint16_t)((vcm_start - 140) / step_bound_region_0);
  } else if (vcm_start > 300 && vcm_start <= 340) {
    step = (uint16_t)((vcm_start - 120) / step_bound_region_0);
  } else if (vcm_start > 100 && vcm_start <= 300) {
    step = (uint16_t)((vcm_start - 100) / step_bound_region_0);
  } else {
    step = 0;
  }

  if (step > 0) {
    init_settings[0].reg_data = step + 10;
    init_settings[1].reg_data = step * 2 - 2;
  } else {
    // Do nothing here
  }
	...
	if (set_info->actuator_params.init_setting_size &&
		...
		if (a_ctrl->func_tbl->actuator_init_focus) {
			...
			if(1 == actuator_bu64291){				
				...
				if(rc < 0) pr_err("ov8835 af calibration falied!!!\n");				
			}
			#endif
	...
	void __user *argp)
{
	...
	#if defined(CONFIG_AR0542) // distinguish actuator maker id, weilanying add
	int cam_name = 0;
	if (2 == actuator_bu64291){
		get_actuator_cam_name((int)&cam_name);
		a_ctrl->cam_name = cam_name;
 ...
 */
	...
	rc = of_device_is_available(client->dev.of_node);
	if (0 == rc) {
		pr_err("failed rc %d\n", rc);
		return -1;
	}
 ...
 */
#if 1
	if (of_property_read_bool(client->dev.of_node, "qcom,actuator-name" ) ==
		true) {
		rc = of_property_read_string(client->dev.of_node, "qcom,actuator-name",
				&actuator_name);
		if (rc < 0) {
			pr_err("failed rc %d\n", rc);
			return rc;
	...
	#if  defined(CONFIG_AR0542) // distinguish actuator maker id, weilanying add
		else if (!strcmp(actuator_name,"dw9714"))
		{
			actuator_bu64291 = 2;
		}
		#endif
