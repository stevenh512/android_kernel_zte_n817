...
//heq
  ...
  #define SP0A20_P1_0x14		0x70		//c4 //kl_outdoor 
  #define SP0A20_P1_0x15		0x70		//c4 //kl_nr      
  #define SP0A20_P1_0x16		0x6a		//c4 //kl_dummy    
  #define SP0A20_P1_0x17		0x6a		//c4 //kl_low   

//sharpness  
#define SP0A20_P2_0xe8         0x18 //0x10//10//;sharp_fac_pos_outdoor
#define SP0A20_P2_0xec         0x18 //0x20//20//;sharp_fac_neg_outdoor

#define SP0A20_P2_0xe9         0x18 //0x0a//0a//;sharp_fac_pos_nr
#define SP0A20_P2_0xed         0x18 //0x20//20//;sharp_fac_neg_nr

#define SP0A20_P2_0xea         0x18 //0x08//08//;sharp_fac_pos_dummy
#define SP0A20_P2_0xee         0x18 //0x18//18//;sharp_fac_neg_dummy

#define SP0A20_P2_0xeb         	0x18 //0x08//08//;sharp_fac_pos_low
#define SP0A20_P2_0xef          0x18 //0x08//18//;sharp_fac_neg_low
 

//saturation
#define SP0A20_P1_0xd3		0x6c
#define SP0A20_P1_0xd4		0x78
#define SP0A20_P1_0xd5  	0x5d
#define SP0A20_P1_0xd6  	0x4b

#define SP0A20_P1_0xd7		0x6c
#define SP0A20_P1_0xd8		0x78
...
static struct msm_sensor_power_setting sp0a20_power_setting[] = {
	{
		...
		.sleep_val = GPIO_OUT_HIGH,//lvcheng 20150327 fix bug:when front camera isn't installed,the rear camera don't work.
		.delay = 5,
	},
	{
	...
	//lvcheng 20141031 giant use L14 now
	/*{
		.seq_type = SENSOR_GPIO,
...
#ifdef CONFIG_BOARD_HERA
		//ae setting  24m 15fps
	{0xfd,0x00},
		{0x03,0x01},
{0x04,0x4a},
{0x05,0x00},
...
{0x73,0x56},
		{0x74,0xaa},
		//tl84
{0x6b,0x10},//0x0e 20150325 wxl
{0x6c,0x2d},
{0x6d,0x36},
{0x6e,0x5a},
		{0x6f,0xaa},
		//f
{0x61,0xeb},//0xf2 20150325 wxl
{0x62,0x10},//0x16 20150325 wxl
{0x63,0x50},//0x5b 20150325 wxl
{0x64,0x7c},
		{0x65,0x6a},
...
int32_t sp0a20_sensor_init_reg_standby(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	printk("\n%s E\n",__func__);
 ...
 int32_t sp0a20_sensor_standby_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	static void *data[10];
	printk("sp0a20_sensor_standby_down\n");
 ...
 int32_t sp0a20_sensor_standby_on(struct msm_sensor_ctrl_t *s_ctrl)
{
	static void *data[10];
	printk("sp0a20_sensor_standby_on\n");
...
static void device_info_show(void)
{
	...
	properties_kobj = kobject_create_and_add("camera_info", NULL);
	if (properties_kobj)
		ret = sysfs_create_group(properties_kobj,
				&properties_attr_group);
	if (!properties_kobj || ret)
		pr_err("failed to create tp_info\n");    
}
/* NiCX add for sp0a20 device info end 20140708 */
int32_t sp0a20_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	uint16_t chipid = 0;
	static int sp0a20_flag=0;
...
static struct msm_sensor_ctrl_t sp0a20_s_ctrl = {
	...
	#ifdef SENSOR_USE_STANDBY
	.sensor_power_down = sp0a20_sensor_standby_down,
	.sensor_power_on = sp0a20_sensor_standby_on,
	.sensor_init_reg = sp0a20_sensor_init_reg_standby
	#endif
};

module_init(sp0a20_init_module);
module_exit(sp0a20_exit_module);
MODULE_DESCRIPTION("sp0a20 YUV sensor driver");
