...
int32_t msm_eeprom_i2c_probe(struct i2c_client *client,
	...
	dev_set_drvdata(&client->dev, &e_ctrl->msm_sd.sd);
	e_ctrl->msm_sd.sd.internal_ops = &msm_eeprom_internal_ops;
	e_ctrl->msm_sd.sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	snprintf(e_ctrl->msm_sd.sd.name,
		ARRAY_SIZE(e_ctrl->msm_sd.sd.name), "%s", id->name);
...
#else
struct spi_driver msm_eeprom_spi_driver = {
#endif
	.driver = {
		.name = "qcom_eeprom",
		.owner = THIS_MODULE,
...
//	rc = spi_register_driver(&msm_eeprom_spi_driver);//weilanying delete
	CDBG("%s:%d spi rc %d\n", __func__, __LINE__, rc);
...
static void __exit msm_eeprom_exit_module(void)
{
	...
	//spi_unregister_driver(&msm_eeprom_spi_driver);//weilanying delete
