...
static int ext_spk_amp_gpio = 18;//songyy
#else
static int ext_spk_amp_gpio = 88;//songyy
#endif
static int ext_spk_boost_gpio =6;//songyy
...
static struct wcd9xxx_mbhc_config mbhc_cfg = {
	...
	.gpio = 0,
	.gpio_irq = 0,
#if defined(CONFIG_BOARD_FAERIE)||defined(CONFIG_BOARD_EROS)||defined(CONFIG_BOARD_IRIS)
	.gpio_level_insert = 1,
#else	
	.gpio_level_insert = 0,
#endif	
...
static void msm8x10_enable_ext_spk_boost_gpio(u32 on);//songyy
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	if (ext_spk_amp_gpio >= 0) {
		if (SND_SOC_DAPM_EVENT_ON(event)){
			...
			msm8x10_enable_ext_spk_boost_gpio(1);//songyy
		}else{
			msm8x10_enable_ext_spk_power_amp(0);
			msm8x10_enable_ext_spk_boost_gpio(0);//songyy
			}
	}
	return 0;
...
static void msm8x10_enable_ext_spk_boost_gpio(u32 on)//songyy
{
	...
	pr_debug("%s: %s external speaker boost.\n", __func__,
			on ? "Enable" : "Disable");
}
...
#define S(X, Y) ((WCD9XXX_MBHC_CAL_PLUG_TYPE_PTR(msm8x10_wcd_cal)->X) = (Y))
	S(v_no_mic, 68); // chenjun:orig:30->398(@1.8V)->686(@2.7V)
