...
// chenjun
	if ((mbhc->in_swch_irq_handler) || (mbhc->current_plug != PLUG_TYPE_HEADSET))
	{
	    pr_err("%s: Switch irq kicked in or No headset, ignore\n", __func__);
	}
	else
	{
...
static int wcd9xxx_is_false_press(struct wcd9xxx_mbhc *mbhc)
{
	...
	if (r > 0) {
	    r = 0;
	    pr_err("chenjun:%s:recover false press to true:i(%d):mb_v(%d)\n", __func__, i, mb_v); // chenjun
	}
				  ...
				  const s32 micmv)
{
	...
	if (btn == -1) {
		pr_debug("%s: couldn't find button number for mic mv %d\n",
			 __func__, micmv);
		btn = 0;
		pr_err("chenjun:%s:recover micmv(%d) as btn(%d)\n", __func__, micmv, btn); // chenjun
	}
...
static void wcd9xxx_enable_mbhc_txfe(struct wcd9xxx_mbhc *mbhc, bool on)
{
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mbhc_txfe)
		...
		snd_soc_update_bits(mbhc->codec, WCD9XXX_A_TX_7_MBHC_TEST_CTL,
				    0x40, on ? 0x40 : 0x00);
#else
// chenjun:merge from 8974 to fix can't detect button issue.
