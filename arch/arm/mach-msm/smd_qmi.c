...
extern int zte_qmi_ctrl_wakeup;		//zte jiangfeng add
static void qmi_dump_msg(struct qmi_msg *msg, const char *prefix)
{
	...
	//zte jiangfeng add
	#if 0
	if(zte_qmi_ctrl_wakeup)
	{
		printk(KERN_INFO
			   "qmi: %s: service id=%02x client id=%02x transaction id=%04x message type=%04x message size=%04x\n",
			   prefix, msg->service, msg->client_id,
			   msg->txn_id, msg->type, msg->size);
	}
	zte_qmi_ctrl_wakeup	=	0;
	#endif
