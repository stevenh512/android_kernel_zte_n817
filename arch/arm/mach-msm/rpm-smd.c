...
// copied from rpm support, for reference purpose
#define RPM_ZTE_FEATURE_REQ 0x7265747A
#define MSM_RPM_ID_ZTER_RFCLK4And5_Combined	0
// and id0 subkeys, we have fixed data_len=12bytes
#define RPM_SMD_KEY_SET	0x00746573	// 'set\0', data len=4,value=1 for enable or 0 for disable
#define RPM_SMD_KEY_GET	0x00746567	// 'get\0' data len=4, value=0 for nothing meaning here
/*typedef struct __attribute__((packed, aligned(4)))
{
	unsigned int key_type;
	unsigned int key_len;
	unsigned int key_value;
} rpm_zte_feature_request_0_t;*/
// copied from rpm support end

#define INNER_ZERO_BYTE_LEN	 0
#define INNER_SINGLE_BYTE_LEN	 1
#endif
...
#if defined (CONFIG_ZTE_SUPPORT_RPM_CMD_FOR_RFCLK45_COMBINED)
// hook for zte extended rpm cmd, which ret a single error byte as a mode status flag
// the qualcomm default rc=-ENODEV and 0 represent ok
// ATTENTION only zte use a single byte case here
		pr_info("rfclk45: if me? msg_id=%d (0x%x, len=%d, %d)\n",  // for debug only
			((struct msm_rpm_ack_msg *)buf)->id_ack,
			*(uint32_t *)(buf + sizeof(struct msm_rpm_ack_msg)),	// ERR value
			*(uint32_t *)(buf + sizeof(struct msm_rpm_ack_msg) + 4),	// len=1
			*tmp);
		if(*(uint32_t *)(buf + sizeof(struct msm_rpm_ack_msg) + 4) == INNER_SINGLE_BYTE_LEN)
		{	// yes, a single byte returned, it must be me
			rc = *tmp;
		}
		else if(*(uint32_t *)(buf + sizeof(struct msm_rpm_ack_msg) + 4) == INNER_ZERO_BYTE_LEN)
		{	// yes, not a byte returned, it must be me, and value is 0, ATTENTION the *tmp is undetermined
			rc = 0;
		}
		// usually below pr_err will not print except me
#endif	
...
#if defined (CONFIG_ZTE_SUPPORT_RPM_CMD_FOR_RFCLK45_COMBINED)
// inner state
#define INNER_UNINITIALIZED	0xFF
#define INNER_INVALID_1		0xFE
#define INNER_INVALID_2		0xFD
#define INNER_INVALID_RETRY	0xFC

// this var normally contains three bits:
// bit 0 for enable bit
// bit 1 for status 1
// bit 2 for status 2

#define INNER_INVALID_MASK		0xF0

#define INNER_ENABLE_CMD_VALUE	1
#define INNER_DISABLE_CMD_VALUE	0

int rfclk45_mode = INNER_UNINITIALIZED;	// if get 0xFF means not updated yet, this is a inner state var
// ATTENTION: must make sure the rpm smd already inited by judge: "msm_rpm_data.ch_info" is not null
bool inner_rpm_smd_initialized(void)
{
	return (msm_rpm_data.ch_info);	// if not null. means inited
}
// ASSIST function to set or get the rpm feature targeted, can be extern type, noirq is a kind of  sync mode
// and only allowed during early boot stage
unsigned char rfclk45_request_noirq(unsigned int value)	// always update the param var
{
	struct msm_rpm_kvp kvp = {
		.key = RPM_SMD_KEY_SET,	// always for SET
		.data = (void *)&value,
		.length = sizeof(value),
	};

	if(!inner_rpm_smd_initialized())
		return INNER_INVALID_1; // that is a invalid value

	pr_info("%s starting: new value =%d / origin status=%d\n", __func__, value, rfclk45_mode);

	// current the rpm side allowed ACTIVE SET or SLEEP set, both ok
	// And below is a synchronous process, while until ack, not allowd failure
	// we expected a rpm target mode status single byte value, then update, print, and return
	rfclk45_mode = msm_rpm_send_message_noirq(MSM_RPM_CTX_ACTIVE_SET,
			RPM_ZTE_FEATURE_REQ, MSM_RPM_ID_ZTER_RFCLK4And5_Combined, &kvp, 1);

	pr_info("%s(%d): ret=%d\n", __func__, value, rfclk45_mode);
	return rfclk45_mode;
}

// This is a asynchronous block mode
unsigned char rfclk45_request(unsigned int value)	// always update the param var
{
	struct msm_rpm_kvp kvp = {
		.key = RPM_SMD_KEY_SET,	// always for SET
		.data = (void *)&value,
	...
	// And below is a synchronous process, while until ack, not allowd failure
	// we expected a rpm target mode status single byte value, then update, print, and return
	rfclk45_mode = msm_rpm_send_message(MSM_RPM_CTX_ACTIVE_SET,
			RPM_ZTE_FEATURE_REQ, MSM_RPM_ID_ZTER_RFCLK4And5_Combined, &kvp, 1);
...
// not used yet, for power on set will update the target var, async mode, so not allowed in early stage of boot
unsigned char rfclk45_get(void)
{
	unsigned int value=0;
	struct msm_rpm_kvp kvp = {
		.key = RPM_SMD_KEY_GET,	// always for GET
		.data = (void *)&value,	// rpm not care the content, just be 4bytes len ok
		.length = sizeof(value),
	};

	if(!inner_rpm_smd_initialized())
		return INNER_INVALID_2; // that is a invalid value

	// current the rpm side allowed ACTIVE SET or SLEEP set, both ok
	// And below is a synchronous process, while until ack, not allowd failure
	// we expected a rpm target mode status single byte value
	rfclk45_mode = msm_rpm_send_message(MSM_RPM_CTX_ACTIVE_SET,
			RPM_ZTE_FEATURE_REQ, MSM_RPM_ID_ZTER_RFCLK4And5_Combined, &kvp, 1);
	pr_info("%s(%d): ret=%d\n", __func__, value, rfclk45_mode);
	return rfclk45_mode;
...
// return rpm state, ths sys will trigger 2 times of read, so we got 2 times of rfclk45_get below
static ssize_t rfclk45_debugcommand_read(struct file *file, char __user *user_buf,
			     size_t count, loff_t *ppos)
{
	char retbuf[50];	// make sure enough space when compile
	unsigned int len = 0, size = 50;
	ssize_t retval = 0;

	len += snprintf(retbuf + len, size - len, "RFCLK mode %d\n", rfclk45_get());
	retval = simple_read_from_buffer(user_buf, count, ppos, retbuf, len);
	return retval;
}

// support char* command type, like enable/disable
static ssize_t rfclk45_debugcommand_write(struct file *filp,
		const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	pr_debug("%s:%s\n", __func__, cmp);
	if (!strcmp(cmp, "enable")) {	// not implement currently
		// call enable write command
		rfclk45_request(INNER_ENABLE_CMD_VALUE);
	} else if (!strcmp(cmp, "disable")) {
		rfclk45_request(INNER_DISABLE_CMD_VALUE);
	} else {
		return -EINVAL;
	}
...
// support for "sys_debugcommand" write only file interface
static const struct file_operations rfclk45_debug_fops = {
	.open	= simple_open,
	.read	= rfclk45_debugcommand_read,
	.write	= rfclk45_debugcommand_write,
};

// ADD ZTE extended RPM command sys device interface, which will be created in the late_initcall steps
static int rfclk45_debugfs_init(void)
{
	static struct dentry *rfclk45_base_dir;
	struct dentry *rfclk45_file1;
	int ret = 0;

	rfclk45_base_dir = debugfs_create_dir("rfclk45_dir", NULL);
	if(rfclk45_base_dir)	// add sub files
	{
		rfclk45_file1 = debugfs_create_file("state",
				S_IRUGO|S_IWUSR, rfclk45_base_dir, NULL, &rfclk45_debug_fops);	// directly use the globle data
		
		if(!rfclk45_file1)
		{
			debugfs_remove(rfclk45_base_dir);
			ret = -ENOMEM;
			return ret;
		}
	}
