...
bool ac_charger_control_lpmclock=false;//////zte_pm_lkj add code for LPM clock open during AC charge
...
static bool msm_mpm_interrupts_detectable(int d, bool from_idle)
{
	...
	/*zte_pm_lkj_start add code for LPM clock open during AC charge*/
	if (ac_charger_control_lpmclock)
	{
		if(debug_mask||(!from_idle))
			pr_info("LPM clock open during ac charge\n");
		ret = false;
	}
	/*zte_pm_lkj_end add code for LPM clock open during AC charge*/
	return ret;
}
